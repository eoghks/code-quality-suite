...
```
rules/
├── shared-standards.md        (공통 표준 — 레이어·Lombok·네이밍)
├── refactor-rules.md          (Refactor Agent — CC·메서드 크기·설계 패턴)
├── quality-rules.md           (Quality Agent — 커버리지·성능·SpotBugs/JaCoCo)
├── security-rules.md          (Security Agent — OWASP Top 10 인덱스)
├── architecture-rules.md      (Architecture Agent — 레이어·DDD·@Transactional)
├── migration-rules.md         (DB Migration Agent — DROP/NOT NULL/버전)
├── suppress-policy.md         (공통 — @suppress 문법·허용 코드)
├── baseline-policy.md         (공통 — Baseline v2 포맷·만료 정책)
├── static-analysis-tools.md   (Quality Agent — PMD/Checkstyle/OWASP-DC 파싱)
├── prompt-safety.md           (공통 — Prompt Injection 방어)
└── security/                  (Security Agent 카테고리별 세부 규칙)
    ├── injection.md
    ├── crypto.md
    ├── access-control.md
    ├── deserialization.md
...
오버라이드 대상으로 가장 자주 쓰이는 파일:
- `shared-standards.md` — 레이어 패턴, Lombok 허용 범위
- `refactor-rules.md` — CC 임계값, 메서드 최대 줄 수
- `quality-rules.md` — JaCoCo 커버리지 임계값, N+1 감지 완화
- `security-rules.md` — 추가 Secret 키워드, SSRF 허용 도메인
- `suppress-policy.md` — Stale 기준일 조정 (기본 180일)
